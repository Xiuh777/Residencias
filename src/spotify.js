...
async function getAIRecommendation(userPrompt) {
    try {
        ...
        return { searchTerms: userPrompt, hexColor: "#1db954" }; 
    }
}
...
async function fetchSpotify(token, query) {
    const randomOffset = Math.floor(Math.random() * 5); 
    const url = `https://api.spotify.com/v1/search?q=${encodeURIComponent(query)}&type=playlist&limit=10&offset=${randomOffset}`;
...
export async function searchPlaylistsByMood(query, skipAI = false) {
  ...
  try {
    ...
    let items = await fetchSpotify(token, searchTerms);

    if (items.length === 0 && searchTerms !== query) {
        items = await fetchSpotify(token, query);
    }

    return { 
        items: shuffleArray(items).slice(0, 5), 
        aiTerms: searchTerms, 
        moodColor: hexColor 
    };

  } catch (error) {
    return { items: [], aiTerms: query, moodColor: "#1db954" };
  }
}
